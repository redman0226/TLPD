...
def main(args):
    ...
    json_dict = {}
    split_image_id = 0
    for image_name, imagedict in tqdm(panda_annos.items()):
        ...
        if splitting_strategy == 'uniform':
            ...
            while left < imgwidth:
                ...
                while up < imgheight:
                    ...
                    json_dict[image_name].append([left, up, right, down])
                    if up + subimage_height >= imgheight:
                        break
                    else:
                        up = up + slide_height
    ...
    json_output_path = join(output_dir, 'split_boxbased.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-od', '--output_dir', type=str, default='./res/uniform/', help='')
    parser.add_argument('-gt', '--groudtruth_path', type=str, default='//192.168.128.37/share/kv_zhao/datasets/panda/annoJSONs/person_bbox_train.json')
    parser.add_argument('--scale', type=float, default=1, help='Image rescale')
