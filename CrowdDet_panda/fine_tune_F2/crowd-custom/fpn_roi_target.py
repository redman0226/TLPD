...
def fpn_roi_target(rpn_rois, rpn_rois_inds, im_info, gt_boxes, top_k_default=1):
    return_rois = []
    return_labels = []
    return_bbox_targets = []
    ...
    for bid in range(config.train_batch_per_gpu):
        ...
        batch_inds = torch.ones((gt_boxes_perimg.shape[0], 1)).type_as(gt_boxes_perimg) * bid #1,1
        #if config.proposal_append_gt:
        gt_rois = torch.cat([batch_inds, gt_boxes_perimg[:, :4]], axis=1) #1,1 cat 1,4 = 1,5
        
        if bid == 0:
            all_rois = torch.cat([rpn_rois[:rpn_rois_inds[0]], gt_rois], axis=0) 
        else:
            all_rois = torch.cat(
                    [rpn_rois[rpn_rois_inds[bid - 1]:rpn_rois_inds[bid]],
                     gt_rois], axis=0)

        overlaps_normal, overlaps_ignore = box_overlap_ignore_opr(
                all_rois[:, 1:5], gt_boxes_perimg) 
        overlaps_normal, overlaps_normal_indices = overlaps_normal.sort(descending=True, dim=1)#2001,1 2039,39
        ...
        max_overlaps_normal = overlaps_normal[:, :top_k].flatten() #2001(???) 4078(2039*2)
